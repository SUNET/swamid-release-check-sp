...
class IdPCheck {
   ...
   */
  protected function __construct5($test, $testname, $testtab, $expected, $nowarn) {
    $this->test = $test;
  ...
  protected function setupAssurance(array &$attributes, array &$ac) {
    $this->RAFAttributes = array(
      "https://refeds.org/assurance"              => array ('level' => 'AL1', "status" => "Missing"),
    ...
    if (isset($attributes["eduPersonAssurance"])) {
      foreach (explode(";",$attributes["eduPersonAssurance"]) as $ALevel) {
        switch ($ALevel) {
          case 'https://refeds.org/assurance/IAP/low' : # NOSONAR Should be http://
            ...
            break;
          case 'https://refeds.org/assurance/IAP/medium' : # NOSONAR Should be http://
            ...
            break;
          case 'https://refeds.org/assurance/IAP/high' : # NOSONAR Should be http://
  ...
  protected function checkRAF(array &$attributes, array &$ac) {
    ...
    if ($this->notAllowed) {
      ...
      $this->status["error"] .= "Missing Assurance information. Expected at least https://refeds.org/assurance<br>";
      $this->status["testResult"] = "Missing https://refeds.org/assurance for user.";
    } elseif ($missing) {
  ...
  protected function checkMFA(array &$attributes, array &$ac) {
    ...
    $this->showRAFAttributeStatus('RAF Low status', 'https://refeds.org/assurance/IAP/low');
    $this->showRAFAttributeStatus('RAF Medium status', 'https://refeds.org/assurance/IAP/medium');
    $this->showRAFAttributeStatus('RAF High status', 'https://refeds.org/assurance/IAP/high');
  ...
  protected function showRAFAttributeStatus($text, $attributeValue) {
    if ($this->RAFAttributes[$attributeValue]['status'] <> 'NotExpected') {
